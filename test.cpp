...
int main() {
    ...
    grade = 0;
    DoublyListTest list_test;

    string Doubly_test_descriptions[10] = {
            "Test1: New empty list is valid",
            "Test2: insert_front() and insert_back() on zero-element list",
            "Test3: select() and search() work properly",
            "Test4: remove_front() and remove_back() on one-element list",
            "Test5: replacing a node's value",
            "Test6: insert_front() keeps moving elements forward",
            "Test7: inserting at different positions in the list",
            "Test8: try to remove too many elements, then add a few elements",
            "Test9: lots of inserts and deletes, all of them valid",
    ...
    bool doubly_test_results[10];
    doubly_test_results[0] = list_test.test1();
    doubly_test_results[1] = list_test.test2();
    doubly_test_results[2] = list_test.test3();
    doubly_test_results[3] = list_test.test4();
    doubly_test_results[4] = list_test.test5();
    doubly_test_results[5] = list_test.test6();
    doubly_test_results[6] = list_test.test7();
    doubly_test_results[7] = list_test.test8();
    doubly_test_results[8] = list_test.test9();
    doubly_test_results[9] = list_test.test10();

    cout << "DOUBLY LINKED LIST TESTING RESULTS \n";
    cout << "******************************* \n";
    for (int i = 0; i < 10; ++i) {

        cout << Doubly_test_descriptions[i] << endl
             << get_status_str(doubly_test_results[i]) << endl;

        if (doubly_test_results[i])
            grade += 3;
    }
    grade += 10;
    cout << endl
...
bool DoublyListTest::test2()
{
    auto *list1 = new DoublyLinkedList();
...
bool DoublyListTest::test3() {
    auto *list1 = new DoublyLinkedList();
    unsigned int capacity = 5;


    for (unsigned int i = 0; i < capacity; i++) {
        ASSERT_TRUE(list1->insert_back(i * 100))
    }

    unsigned int idx = 3;
    ASSERT_TRUE(list1->search(list1->select(idx)) == idx)
    ASSERT_TRUE(list1->select(capacity + 1) == list1->select(capacity))
    ASSERT_TRUE(list1->search(1000) == list1->size())

    for (unsigned int i = 0; i < capacity; i++) {
       ASSERT_TRUE(list1->select(i) == i*100)
    }

    return true;
}
bool DoublyListTest::test4() {
    auto *list1 = new DoublyLinkedList();
    auto *list2 = new DoublyLinkedList();
...
bool DoublyListTest::test5() {
    auto *list1 = new DoublyLinkedList();

    ASSERT_TRUE(list1->insert_front(100))
    ASSERT_TRUE(list1->insert_front(115))
    ASSERT_TRUE(list1->insert_back(80))

    ASSERT_TRUE(list1->size() == 3)

    ASSERT_TRUE(list1->replace(1,160))

    return true;
}
bool DoublyListTest::test6() {
    auto *list = new DoublyLinkedList();
    unsigned int capacity = 5;
    for (unsigned int i = 0; i < capacity; i++) {
